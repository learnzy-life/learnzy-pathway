...
const ImprovementResources: React.FC<ImprovementResourcesProps> = ({
  resources,
  bioResources,
}) => {
    ...
    <div className="card-glass p-4 sm:p-6">
      <div className="mb-4 sm:mb-6">
        <div className="flex items-center mb-3 sm:mb-4">
          ...
          <h3 className="text-base sm:text-lg font-medium">
            Topics With Highest Improvement Potential
          </h3>
        </div>
        <p className="text-xs sm:text-sm text-muted-foreground mb-4 sm:mb-6">
          We've identified these topics where focused study will have the
          greatest impact on your score. Topics are prioritized based on your
          current accuracy, difficulty level, and exam importance.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {topResources.map((item, index) => {
          ...
          console.log(
            `Topic: ${item.topic}, Matching resource:`,
            matchingResource
          )
          ...
          return (
            <div
              key={index}
              className="bg-white rounded-xl border border-gray-100 shadow-subtle overflow-hidden"
            >
              <div className="p-3 sm:p-5 border-b border-gray-100">
                <div className="flex justify-between items-center mb-2 sm:mb-3">
                  <h4 className="text-sm sm:font-medium text-learnzy-dark">
                    {item.topic}
                  </h4>
                  <span
                    className={`text-xs sm:text-sm font-medium ${getAccuracyColor(
                      item.accuracy
                    )}`}
                  >
                    {item.accuracy}%
                ...
                <div className="flex flex-wrap gap-1 sm:gap-2 mb-2 sm:mb-3">
                  {item.difficultyLevel && (
                    <span
                      className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full ${getDifficultyColor(
                        item.difficultyLevel
                      )}`}
                    >
                      {item.difficultyLevel}
                  ...
                  {item.priorityLevel && (
                    <span
                      className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full ${getPriorityColor(
                        item.priorityLevel
                      )}`}
                    >
              ...
              <div className="px-3 sm:px-5 py-2 sm:py-3">
                <h5 className="text-xs sm:text-sm font-medium text-learnzy-dark mb-2 sm:mb-3">
                    ...
                    matchingResource.ncert_link === 'NA' ? (
                      <div className="flex items-start p-1.5 sm:p-2 rounded-lg bg-gray-50">
                        ...
                        <div className="flex-1">
                          <div className="text-xs sm:text-sm font-medium text-learnzy-dark">
                      ...
                      <a
                        href={
                          ...
                          `https://learnzy.co.in/resources/biology/ncert/${item.topic
                            .toLowerCase()
                            .replace(/\s+/g, '-')}`
                        }
                    ...
                    matchingResource.video_link === 'NA' ? (
                      ...
                      <a
                        href={
                          ...
                          `https://learnzy.co.in/resources/biology/video/${item.topic
                            .toLowerCase()
                            .replace(/\s+/g, '-')}`
                        }
                        className="flex items-start p-1.5 sm:p-2 rounded-lg hover:bg-gray-50 transition-colors"
      ...
      <div className="flex items-center justify-center mt-6 sm:mt-8 p-3 sm:p-4 bg-green-50 rounded-lg border border-green-100">
        <div className="text-center">
          ...
          <div className="text-xs sm:text-sm text-green-600 mt-1">
            We've prioritized topics where you'll see the biggest gains with the
            least effort
          </div>
        </div>
      </div>
