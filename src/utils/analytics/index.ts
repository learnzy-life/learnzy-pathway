...
import {
  getDefaultCognitiveInsights,
  getDefaultImprovementResources,
  getDefaultMindsetAnalysis,
} from './defaultData'
...
export const calculateAnalytics = async (
  ...
  const totalTimeSeconds = userAnswers.reduce(
  ...
  if (subj === 'biology') {
    try {
      console.log('Fetching resources from bio_resources table...')
